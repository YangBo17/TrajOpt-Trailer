...
garage_length = 1.0
# waypoints
waypoints = [0.0 0.0; 2.3 -0.25; 1.05 -0.75; 1.15 -0.75 ]'
start_vel = [1.0, 0.0]
goal_vel = [0.0, 0.0]
# plot env
function plot_env()
    ...
    xlims!(fig_env, -1.0, 3.5)
    # obstacle
    plot!(fig_env, rectangle(2,1.5,-1,-2), opacity=0.5, color=:gray, label="")
    plot!(fig_env, rectangle(2,1.5,2,-2), opacity=0.5, color=:gray, label="")
    plot!(fig_env, rectangle(1,1.0,1,-2), opacity=0.5, color=:gray, label="")
    plot!(fig_env, rectangle(4.5,1.5,-1,0.5), opacity=0.5, color=:gray, label="")
    # middle points
...
function trajopt_space(Time::Vector{Float64})
    ...
    for i in 1:N-1
        if i == 1 
            ...
            @constraint(model, D1p[1,i](t=>Time[i]) == 0.0)
