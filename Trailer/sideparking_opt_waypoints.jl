...
function plot_env()
    ...
    # plot!(fig_env, rectangle(0.5*road_unit,0.5*road_unit,1.0,0.3*road_unit), opacity=0.5, color=:blue, label="")
    # middle points
...
function trajopt_space(Time::Vector{Float64})
    ...
    for i in 1:N-1
        if i == 1 
            ...
            @constraint(model, D1p[1,i](t=>Time[i]) == 2.0)
            @constraint(model, D1p[2,i](t=>Time[i]) == 0.0)
            # @constraint(model, D2p[1,i](t=>Time[i]) == 0.1)
            # @constraint(model, D2p[2,i](t=>Time[i]) == 0.1)
        elseif i == 3
            @constraint(model, p[1,i](t=>Time[i]) == p[1,i+1](t=>0.0))
            @constraint(model, p[2,i](t=>Time[i]) == p[2,i+1](t=>0.0))
            @constraint(model, D1p[1,i](t=>Time[i]) == -D1p[1,i+1](t=>0.0))
