...
# waypoints
waypoints = [-1.0*road_unit 0.5*road_unit; 0.8*road_unit 0.3*road_unit; 0.0 0.0; 0.0 -garage_length+0.15]'
start_vel = [1.0, 0.0]
goal_vel = [0.0, 0.0]
# plot env
function plot_env()
...
function trajopt_space(Time::Vector{Float64})
    ...
    for i in 1:N-1
        if i == 1 
            ...
            @constraint(model, D1p[1,i](t=>Time[i]) == 0.2)
            @constraint(model, D1p[2,i](t=>Time[i]) == 0.1)
    ...
    @variable(model, f[1:6], Poly(Xf))
    @variable(model, g[1:6], Poly(Xg))
    for i in 1:6
    ...
    if d%2 == 0
        @constraint(model, p[2,1] - 0.0 == f[1] - (t-0.0)*(t-Time[1])*g[1])
        @constraint(model, - p[2,1] + road_width == f[2] - (t-0.0)*(t-Time[1])*g[2])
        @constraint(model, p[2,2] - 0.0 == f[3] - (t-0.0)*(t-Time[2])*g[3])
        @constraint(model, - p[2,2] + road_width == f[4] - (t-0.0)*(t-Time[2])*g[4])
        @constraint(model, p[1,3] - (-garage_width/2) == f[5] - (t-0.0)*(t-Time[3])*g[5])
        @constraint(model, - p[1,3] + garage_width/2 == f[6] - (t-0.0)*(t-Time[3])*g[6])
    else
        @constraint(model, p[2,1] - 0.0 == (t-0.0)*f[1] - (t-Time[1])*g[1])
        @constraint(model, - p[2,1] + road_width == (t-0.0)*f[2] - (t-Time[1])*g[2])
        @constraint(model, p[2,2] - 0.0 == (t-0.0)*f[3] - (t-Time[2])*g[3])
        @constraint(model, - p[2,2] + road_width == (t-0.0)*f[4] - (t-Time[2])*g[4])
        @constraint(model, p[1,3] - (-garage_width/2) == (t-0.0)*f[5] - (t-Time[3])*g[5])
        @constraint(model, - p[1,3] + garage_width/2 == (t-0.0)*f[6] - (t-Time[3])*g[6])
    end
...
visual_traj(fig_env, data1; num=1, c0=:orange, c1=:green, label="opt")
