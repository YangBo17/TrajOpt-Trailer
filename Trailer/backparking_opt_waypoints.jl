...
function trajopt_space(Time::Vector{Float64})
    ...
    @variable(model, 0.30*road_unit <= b2 <= 0.80*road_unit)
    ...
    for i in 1:N
        if i!= 2
            ...
            @constraint(model, p[1,i](t=>0.0) == a2)
            @constraint(model, p[2,i](t=>0.0) == b2)
        end
    end
    ...
    @constraint(model, D1p[1,3](t=>0.0) == 0.0)
    @constraint(model, D1p[2,3](t=>0.0) == -0.5)
    @constraint(model, D1p[1,end](t=>0.0) == 0.0)
    @constraint(model, D1p[2,end](t=>0.0) == -0.5)
    ...
    for i in 1:N-1
        if i == 1 
            ...
            @constraint(model, D1p[1,i](t=>Time[i]) == 1.4)
            @constraint(model, D1p[2,i](t=>Time[i]) == -0.0)
            # @constraint(model, D2p[1,i](t=>Time[i]) == 0.0)
    ...
    if d%2 == 0
        for i in 1:8
            @constraint(model, rect_new[i,2][1] - (-0.1) == f[1,i] - (t-0.0)*(t-Time[1])*g[1,i])
            @constraint(model, -rect_new[i,2][1] + road_width == f[2,i] - (t-0.0)*(t-Time[1])*g[2,i])
            @constraint(model, rect_new[i,2][2] - (-0.5) == f[3,i] - (t-0.0)*(t-Time[2])*g[3,i])
            @constraint(model, -rect_new[i,2][2] + road_width == f[4,i] - (t-0.0)*(t-Time[2])*g[4,i])
            @constraint(model, rect_new[i,1][3] - (-garage_width/2) == f[5,i] - (t-0.0)*(t-Time[3])*g[5,i])
            @constraint(model, -rect_new[i,1][3] + garage_width/2 == f[6,i] - (t-0.0)*(t-Time[3])*g[6,i])
            @constraint(model, rect_new[i,1][4] - (-garage_width/2) == f[7,i] - (t-0.0)*(t-Time[4])*g[7,i])
            @constraint(model, -rect_new[i,1][4] + garage_width/2 == f[8,i] - (t-0.0)*(t-Time[4])*g[8,i])
        end
    else
        for i in 1:8
            # @constraint(model, rect_new[i,2][1] - (-0.1) == (t-0.0)*f[1,i] - (t-Time[1])*g[1,i])
            # @constraint(model, -rect_new[i,2][1] + road_width+1.0 == (t-0.0)*f[2,i] - (t-Time[1])*g[2,i])
            # @constraint(model, rect_new[i,2][2] - (-3.0) == (t-0.0)*f[3,i] - (t-Time[2])*g[3,i])
            # @constraint(model, -rect_new[i,2][2] + road_width+3.0 == (t-0.0)*f[4,i] - (t-Time[2])*g[4,i])
            # @constraint(model, rect_new[i,1][3] - (-garage_width/2) == (t-0.0)*f[5,i] - (t-Time[3])*g[5,i])
            # @constraint(model, -rect_new[i,1][3] + garage_width/2 == (t-0.0)*f[6,i] - (t-Time[3])*g[6,i])
            # @constraint(model, rect_new[i,1][4] - (-garage_width/2) == (t-0.0)*f[7,i] - (t-Time[4])*g[7,i])
            # @constraint(model, -rect_new[i,1][4] + garage_width/2 == (t-0.0)*f[8,i] - (t-Time[4])*g[8,i])
        end
    end
