...
# waypoints
waypoints = [0.0 -garage_length; 0.0 -trailer_length; 0.0 0.0; 0.7*road_unit 0.5*road_unit; 1.2*road_unit 0.5*road_unit]'
start_vel = [0.0, +1.0]
goal_vel = [+1.0, 0.0]
# plot env
function plot_env()
...
##
data = data1
fig_v = plot([data[i].t for i in eachindex(data)], [data[i].v for i in eachindex(data)], label="v1")
...
using DataFrames
df = DataFrame(data1)
CSV.write("data_backparking.csv", df)
