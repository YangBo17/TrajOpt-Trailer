...
# waypoints
waypoints = [-1.0*road_unit 0.65*road_unit; 0.5*road_unit 0.35*road_unit; 0.5*road_unit+0.5 0.35*road_unit; 0.5*road_unit 0.35*road_unit; 
            0.0 0.0; 0.0 -trailer_length; 0.0 -garage_length+0.5; 0.0 -garage_length]'
start_vel = [2.0, 0.0]
goal_vel = [0.0, -0.5]
#=
1. 7段轨迹包括3个低速区
2. 长度分别大约为： 1.989m, 0.5m(低速), 0.5m(低速), 0.793m, 0.5m, 0.3m, 0.5m(低速)
=#
# plot env
function plot_env()
    ...
    xlims!(fig_env, -2.0, +2.0+1.0)
    # obstacle
    plot!(fig_env, rectangle(2-garage_width/2,2,-2,-2), opacity=0.5, color=:gray, label="")
    plot!(fig_env, rectangle(2-garage_width/2,2,garage_width/2,-2), opacity=0.5, color=:gray, label="")
    # plot!(fig_env, rectangle(garage_width,2.0-garage_length,-0.5,-2), opacity=0.5, color=:gray, label="")
    plot!(fig_env, rectangle(4,2,-2,garage_length), opacity=0.5, color=:gray, label="")
    # plot!(fig_env, rectangle(0.5*road_unit,0.5*road_unit,0.5*road_unit,0.3*road_unit), opacity=0.5, color=:blue, label="")
    # middle points
    for i in 2:7
        plot!(fig_env, [waypoints[1,i]], [waypoints[2,i]], seriestype=:scatter, aspect_ratio=:equal, markershape=:rect, markersize=2, color=:blue, label="")
    end
    return fig_env
end
...
function trajopt_space(Time::Vector{Float64})
    ...
    # equality constraint
    # @variable(model, 0.5*road_unit <= a2 <= 1.0*road_unit)
    # @variable(model, 0.3*road_unit <= b2 <= 0.8*road_unit)
    ...
    for i in 1:N
        if i == 2
            ...
            # @constraint(model, D1p[1,i](t=>0.0) == 1.0)
            # @constraint(model, D1p[2,i](t=>0.0) == 0.0)
        elseif i == 3
            @constraint(model, p[1,i](t=>0.0) == waypoints[1,i])
            @constraint(model, p[2,i](t=>0.0) == waypoints[2,i])
            # @constraint(model, D1p[1,i](t=>0.0) == -0.1)
            # @constraint(model, D1p[2,i](t=>0.0) == 0.0)
        elseif i == 4
            @constraint(model, p[1,i](t=>0.0) == waypoints[1,i])
            @constraint(model, p[2,i](t=>0.0) == waypoints[2,i])
    ...
    @constraint(model, D1p[1,end](t=>0.0) == 0.0)
    @constraint(model, D1p[2,end](t=>0.0) == -1.0)
    ...
    # rect_raw = [[0,-trailer_width],[trailer_length,-trailer_width],[trailer_length,trailer_width],[0,trailer_width],
    #             [0,-trailer_width],[trailer_length,-trailer_width],[trailer_length,trailer_width],[0,trailer_width]]
    ...
    # if d%2 == 0
    #     for i in 1:8
    #         @constraint(model, rect_new[i,2][1] - (0.0) == f[1,i] - (t-0.0)*(t-Time[1])*g[1,i])
    #         @constraint(model, -rect_new[i,2][1] + road_width == f[2,i] - (t-0.0)*(t-Time[1])*g[2,i])
    #         @constraint(model, rect_new[i,2][4] - (0.3) == f[3,i] - (t-0.0)*(t-Time[4])*g[3,i])
    #         @constraint(model, -rect_new[i,2][4] + road_width == f[4,i] - (t-0.0)*(t-Time[4])*g[4,i])
    #         @constraint(model, rect_new[i,1][5] - (-garage_width/2) == f[5,i] - (t-0.0)*(t-Time[5])*g[5,i])
    #         @constraint(model, -rect_new[i,1][5] + garage_width/2 == f[6,i] - (t-0.0)*(t-Time[5])*g[6,i])
    #         @constraint(model, rect_new[i,1][6] - (-garage_width/2) == f[7,i] - (t-0.0)*(t-Time[6])*g[7,i])
    #         @constraint(model, -rect_new[i,1][6] + garage_width/2 == f[8,i] - (t-0.0)*(t-Time[6])*g[8,i])
    #     end
    # else
    #     for i in 1:8
    #         @constraint(model, rect_new[i,2][1] - (-0.29) == (t-0.0)*f[1,i] - (t-Time[1])*g[1,i])
    #         @constraint(model, -rect_new[i,2][1] + (road_width+0.25) == (t-0.0)*f[2,i] - (t-Time[1])*g[2,i])
    #         @constraint(model, rect_new[i,2][4] - (-0.45) == (t-0.0)*f[3,i] - (t-Time[4])*g[3,i])
    #         @constraint(model, -rect_new[i,2][4] + road_width == (t-0.0)*f[4,i] - (t-Time[4])*g[4,i])
    #         @constraint(model, rect_new[i,1][5] - (-garage_width/2-0.25) == (t-0.0)*f[5,i] - (t-Time[5])*g[5,i])
    #         @constraint(model, -rect_new[i,1][5] + (garage_width/2+0.25) == (t-0.0)*f[6,i] - (t-Time[5])*g[6,i])
    #         @constraint(model, rect_new[i,1][6] - (-garage_width/2-0.1) == (t-0.0)*f[7,i] - (t-Time[6])*g[7,i])
    #         @constraint(model, -rect_new[i,1][6] + (garage_width/2+0.1) == (t-0.0)*f[8,i] - (t-Time[6])*g[8,i])
    #     end
    # end
...
v1 = 3.0
# Time0 = [2.0, 0.5, 0.5, 0.8, 0.5, 0.3, 0.5] .- 0.2
Time0 = [1.0817950697448664, 0.4661597078653014, 0.4728666299794451, 0.5859353345686706, 0.2677672302196067, 0.2563058518601751, 0.5511104265101011]
...
## 
plot_data(fig_env, data; color=:red, label="opt")

##
data = data0
fig_v = plot([data[i].t for i in eachindex(data)], [sqrt(data[i].vx1^2+data[i].vy1^2) for i in eachindex(data)], label="v1")
fig_κ = plot([data[i].t for i in eachindex(data)], [data[i].vx1 for i in eachindex(data)], label="κ")
# ylims!(fig_κ, -10,10)
