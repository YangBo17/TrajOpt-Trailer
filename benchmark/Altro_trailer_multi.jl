...
file_input = "benchmark/data/NLP2016_trailer_traj_"*data_name*".jld2"
...
model = Trailer{Float64}(d0, d1, Wid)
n,m = size(model)
s0 = SVector(start...)
sf = SVector(goal...)
...
x1 = x0 - d1 * cos.(Xmat[4,:])
y1 = y0 - d1 * sin.(Xmat[4,:])
fig_traj = plot(fig_env, x0, y0, aspect_ratio=:equal, label="altro_tractor", title="Altro-E$seed")
...
filename = "benchmark/data/Altro2019_trailer_traj_"*data_name*".jld2"
@save filename traj

method_name = "Altr"*data_name
