...
# plot env
fig_env_line = plot(aspect_ratio=:equal, ylims=(-1.5,+1.5))
xs = env[1]:0.1:env[2]
plot!(fig_env_line, xs, xs -> br, ribbon=(2,0), fillcolor=:gray, fillopacity=0.3, label="" )
plot!(fig_env_line, xs, xs -> bl, ribbon=(0,2), fillcolor=:gray, fillopacity=0.3, label="" )
plot!(fig_env_line, xs, xs -> br, color=:black, label="")
plot!(fig_env_line, xs, xs -> bl, color=:black, label="")

plot_car_trans!(fig_env_line, car_line, cons_line)
...
function NLP(traj0)
    ...
    @constraints(model, begin
        [k=1:100-1], x[k+1] == x[k] + v[k] * cos(ψ[k]) * Δt
        [k=1:100-1], y[k+1] == y[k] + v[k] * sin(ψ[k]) * Δt
        [k=1:100-1], ψ[k+1] == ψ[k] + v[k] * tan(δ[k]) / L * Δt
        [k=1:100-1], v[k+1] == v[k] + a[k] * Δt

        [k=101:200-1], x[k+1] == x[k] + v[k] * cos(ψ[k]) * Δt
        [k=101:200-1], y[k+1] == y[k] + v[k] * sin(ψ[k]) * Δt
        [k=101:200-1], ψ[k+1] == ψ[k] + v[k] * tan(δ[k]) / L * Δt
        [k=101:200-1], v[k+1] == v[k] + a[k] * Δt

        [k=201:300-1], x[k+1] == x[k] + v[k] * cos(ψ[k]) * Δt
        [k=201:300-1], y[k+1] == y[k] + v[k] * sin(ψ[k]) * Δt
        [k=201:300-1], ψ[k+1] == ψ[k] + v[k] * tan(δ[k]) / L * Δt
        [k=201:300-1], v[k+1] == v[k] + a[k] * Δt
    end)
    @constraints(model, begin
        x[1] == start_line.x
        y[1] == start_line.y
    ...
    @constraints(model, begin
        [k=1:N, i=1:3], (x[k]-car[i].x)^2*R[1,1,i] + (x[k]-car[i].x)*(y[k]-car[i].y)*R[1,2,i] + (x[k]-car[i].x)*(y[k]-car[i].y)*R[2,1,i] + (y[k]-car[i].y)^2*R[2,2,i] >= 1
    end)
...
function MILP(init, ltraj0, H)
    ...
    @constraints(model, begin
        ...
        # car3
        [k=1:H], μ3[k,1] >= cons[3,1] - x[k]
        [k=1:H], μ3[k,1] <= M * z3[k,1]
        [k=1:H], μ3[k,1] <= cons[3,1] - x[k] + M * (1-z3[k,1])

        [k=1:H], μ3[k,2] >= x[k] - cons[3,2]
        [k=1:H], μ3[k,2] <= M * z3[k,2]
        [k=1:H], μ3[k,2] <= x[k] - cons[3,2] + M * (1-z3[k,2])

        [k=1:H], μ3[k,3] >= cons[3,3] - x[k]
        [k=1:H], μ3[k,3] <= M * z3[k,3]
        [k=1:H], μ3[k,3] <= cons[3,3] - x[k] + M * (1-z3[k,3])
        [k=1:H], cons[3,4] - M * (μ3[k,1] + μ3[k,2] + μ3[k,3]) <= y[k]
    end)
    @variables(model, begin
        Θx_pos[1:H] >= 0
        Θx_neg[1:H] >= 0
...
function post_process(traj_line)
    traj_raw = deepcopy(traj_line)
    @show idx1 = argmin((traj_line.x.-4.0).^2)
    @show idx2 = argmin((traj_line.x.-8.0).^2)
    sec1 = range(-4.0, 0.0, length(traj_line.t[1:idx1])) |> collect
    sec2 = range(-2.0, 2.0, length(traj_line.t[idx1+1:idx2])) |> collect
    sec3 = range(0.0, 4.0, length(traj_line.t[idx2+1:end])) |> collect
    for i in eachindex(traj.t)
        if i <= idx1
            traj_raw.t[i] = traj_line.t[i]
            traj_raw.v[i] = traj_line.v[i]
            traj_raw.a[i] = traj_line.a[i]
            traj_raw.δ[i] = traj_line.δ[i]
            traj_raw.ψ[i] = traj_line.ψ[i]
            traj_raw.x[i] = sec1[i]
            traj_raw.y[i] = -2.0 + (traj_line.y[i] - 0.0)
        elseif i <= idx2
            traj_raw.t[i] = traj_line.t[i]
            traj_raw.v[i] = traj_line.v[i]
            traj_raw.a[i] = traj_line.a[i]
            traj_raw.δ[i] = traj_line.δ[i]
            traj_raw.ψ[i] = traj_line.ψ[i] + π/2
            traj_raw.x[i] = 0.0 - (traj_line.y[i] - 0.0)
            traj_raw.y[i] = sec2[i-idx1]
        else
            traj_raw.t[i] = traj_line.t[i]
            traj_raw.v[i] = traj_line.v[i]
            traj_raw.a[i] = traj_line.a[i]
...
traj_raw = post_process(traj)
fig_traj_raw = plot(fig_env, traj_raw.x, traj_raw.y, aspect_ratio=:equal, title="NLP_traj", label="")
...
fig_traj = plot(fig_env_line, traj.x, traj.y, aspect_ratio=:equal, title="pureMILP_traj", label="") 
ylims!(fig_traj, -1.5, 1.5) 

##
@time traj = test_rhMILP(H,N)
fig_traj = plot(fig_env_line, traj.x, traj.y, aspect_ratio=:equal, title="rhMILP_traj", label="")
ylims!(fig_traj, -1.5, 1.5) 
# fig_v = plot(traj.t, traj.v, label="v", legend=:outertopright)
# fig_a = plot(traj.t, traj.a, label="a", legend=:outertopright)
# fig_δ = plot(traj.t, traj.δ, label="δ", legend=:outertopright)
...
##
traj = test_MILP(N)
fig_traj = plot(fig_env_line, traj.x, traj.y, aspect_ratio=:equal, title="MILP_traj", label="")
ylims!(fig_traj, -1.5, 1.5)
