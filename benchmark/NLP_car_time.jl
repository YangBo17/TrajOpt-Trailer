...
fig_astar = plot(fig_env, x_car, y_car, label="astar0")
...
function NLP(traj0)
    ...
    obj = @expression(model,
            sum([(x[k]-gx)^2 for k in 1:N]) +
            sum([(y[k]-gy)^2 for k in 1:N]) +
            sum([(ψ[k]-gψ)^2 for k in 1:N]) +
            sum([(v[k]-gv)^2 for k in 1:N]) +
            sum([a[k]^2 for k in 1:N]) +
            sum([δ[k]^2 for k in 1:N])
    )
    ...
    @show car_x = [car_raw[i].x for i in 1:3]
    @show car_y = [car_raw[i].y for i in 1:3]
    @show car_r = [0.5  for i in 1:3]
    for k in 1:N
...
@save "benchmark/data/NLP2016_car_trajT.jld2" traj
