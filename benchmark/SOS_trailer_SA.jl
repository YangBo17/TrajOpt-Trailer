...
function trajopt_space(Time::Vector{Float64}, trajopt_set::TrajOpt_Set, cons_init::Cons_Init, limits_list::Vector, obs_list::Vector)
    ...
    #     Xg = monomials([t], 0:d-1)
    # end
    # @variable(model, fs[1:N,1:4], Poly(Xf))
    # @variable(model, gs[1:N,1:4], Poly(Xg))
    # for i in 1:N
    #     X = [p[1,i], p[2,i], D1p[1,i], D1p[2,i]]
    #     dist = 0.8
    #     if i != 2
    #         B = [corridors[i,1], corridors[i,1], -corridors[i,2], -corridors[i,2]]
    #         B = hcat(B)
    #         A = [1. 0. 0. 0.;
    #             1. 0. dist 0.;
    #             -1. 0. 0. 0.;
    #             -1. 0. -dist 0.]
    #     else
    #         B = [corridors[i,3], corridors[i,3], -corridors[i,4], -corridors[i,4]]
    #         B = hcat(B)
    #         A = [0. 1. 0. 0.;
    #             0. 1. 0. dist;
    #             0. -1. 0. 0.;
    ...
    #         @constraint(model, gs[i,j] >= 0.0)
    #         if d%2 == 0
    #             @constraint(model, (- B[j] + A[j,:]'*X) == fs[i,j] - (t-0.0)*(t-Time[i])*gs[i,j])
    #         else
    #             @constraint(model, (- B[j] + A[j,:]'*X) == (t-0.0)*fs[i,j] - (t-Time[i])*gs[i,j])
    #         end
    #     end
    # end
