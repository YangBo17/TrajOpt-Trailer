...
function solve_ocp(task::Task, Γ::Corr, traj::Traj_trailer)
    ...
    penalty5 = @expression(model,
                sum([min(0, (x0[k]+L/2*cos(ψ0[k])) - Γ.corr0[1,k])^2 for k in 1:N]) + 
                sum([max(0, (x0[k]+L/2*cos(ψ0[k])) - Γ.corr0[2,k])^2 for k in 1:N]) +
                sum([min(0, (y0[k]+L/2*sin(ψ0[k])) - Γ.corr0[3,k])^2 for k in 1:N]) + 
                sum([max(0, (y0[k]+L/2*sin(ψ0[k])) - Γ.corr0[4,k])^2 for k in 1:N]) +
                sum([min(0, (x1[k]+L/2*cos(ψ1[k])) - Γ.corr1[1,k])^2 for k in 1:N]) + 
                sum([max(0, (x1[k]+L/2*cos(ψ1[k])) - Γ.corr1[2,k])^2 for k in 1:N]) +
                sum([min(0, (y1[k]+L/2*sin(ψ1[k])) - Γ.corr1[3,k])^2 for k in 1:N]) + 
                sum([max(0, (y1[k]+L/2*sin(ψ1[k])) - Γ.corr1[4,k])^2 for k in 1:N])
    )
    gx0 = goal[1]
    gy0 = goal[2]
    gψ0 = goal[3]
    ...
    @objective(model, Min, w_time * Δt*N + w_obj * obj + w_penalty * (penalty1+penalty2+penalty3+penalty4+penalty5))
...
function solve_ocp2(task::Task, map_env::Map, traj::Traj_trailer)
    ...
    set_start_value.(δ0, traj.δ0)
    set_start_value.(v1, traj.v1)
    gx0 = task.init.pos[end,1] + d1*cons_init.vel[end,1]/norm(cons_init.vel[end,:])
    gy0 = task.init.pos[end,2] + d1*cons_init.vel[end,2]/norm(cons_init.vel[end,:])
    gψ0 = atan(cons_init.vel[1,2], cons_init.vel[1,1])
    gψ1 = atan(cons_init.vel[1,2], cons_init.vel[1,1])
    gv0 = sqrt(task.init.vel[end,1]^2+task.init.vel[end,2]^2)
    obj = @expression(model,
                sum([ωx0*(x0[k]-gx0)^2 for k in 1:N]) + 
                sum([ωy0*(y0[k]-gy0)^2 for k in 1:N]) + 
                sum([ωψ0*(ψ0[k]-gψ0)^2 for k in 1:N]) + 
    ...
    @objective(model, Min, w_time * Δt*N + w_obj * obj)
    @constraints(model, begin
        x0[1] == start[1]
        y0[1] == start[2]
        ψ0[1] == start[3]
...
@save "benchmark/data/Li2022T_trailer_traj.jld2" traj
